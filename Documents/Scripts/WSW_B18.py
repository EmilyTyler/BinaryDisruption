...
integ2 = np.zeros(N_a)
for i in range(N_a):
        integ2[i] = integrand2(x_bins[i], x_bins[0], tau[0])
...
for i in range(1,N_t):       
        for j in range(N_a):               
                for k in range(N_a-1):
                        N[i,k] += N[0,j] * quad(integrand2, x_bins[k+1], x_bins[k], args=(x_bins[j], tau[i]))[0]
                        #print('this =', N[0,j] * quad(integrand2, x_bins[k], x_bins[k+1], args=(x_bins[j], tau[i]))[0])
                        #print('this =', N[0,j] * quad(integrand2, x_bins[k], x_bins[k+1], args=(x_bins[j], tau[i]))[1])
...
def integrand3(x_0, x, t):
        return 1.0/t * (x_0/x)**(5.0/4.0) * np.exp(-(x + x_0)/t) * iv(5.0/2.0, 2.0*np.sqrt(x*x_0)/t) 
        #return iv(5.0/2.0, 2.0*np.sqrt(x*x_0)/t) 

integ3 = np.zeros(N_a)
print(x_bins[2])
for i in range(N_a):
        integ3[i] = integrand3(x_bins[0], x_bins[i], tau[1]) 
print('integ3 =', integ3)
print(2.0*np.sqrt(x_bins*x_bins[0])/tau[1])
...
for i in range(1,N_t):
        for j in range(N_a):
                for k in range(N_a):
                        N[i,k] += N[0,j] * quad(integrand3, x_bins[j]+dx[j], x_bins[j], args=(x_bins[k], tau[i]))[0]
print('N =', N)
for i in range(0,N_t,N_t//5):
        plt.loglog(a_bins/au, N[i]/(da/au), label='t={}Gyr'.format(t[i]/(giga*year)))
plt.xlabel('Semi-major axis, au')
