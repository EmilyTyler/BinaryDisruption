...
def binning(v_rms, n_p, N_t, t, X, A, m1, m2, M_p):
        ...
        for i in range(1, N_t):
                ...
                #Semi-major axis
                A[i] = semimajorAxis(X[i], m1, m2)
                plt.plot((X[i,0,1]-X[i,1,1]), (X[i,0,0]-X[i,1,0]))
...
def openCloseBinning(v_rms, n_p, N_t, t, A, es, m1, m2, M_p):
        ...
        for i in range(1, N_t):
                ...
                if np.size(i_enc[0]) > 0:
                        for k in range(np.size(i_enc[0])):
                                for l in range(N[i_enc[0,k], i_enc[1,k]]):
                                        (A[i], es[i]) = openCloseEncounter(m1, m2, v[i_enc[1,k]], b[i_enc[0,k]], A[i-1], es[i-1], M_p)
                else:
                        A[i] = A[i-1]
                        es[i] = es[i-1]
...
#Implement encounters with relative velocity v and impact parameter b using impulse approximation, M_p is perturber mass
def openCloseEncounter(m1, m2, v, b, a, e, M_p):
        #print('ENCOUNTER!')      
        #print('b = ', b)
        #print('v = ', v)
        #print('M_p = ', M_p)
