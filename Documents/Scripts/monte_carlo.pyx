...
def MCEncounters(double v_rms, double n_p, double T, double m1, double m2, double M_p, double a, double e):
        ...
        cdef int N = int(T*encounterRate(n_p, v_rms, b_min, b_max, v_min, v_max))
        #Impact parameters of encounters
        cdef np.ndarray b = draw_b(b_max, N)
        #Relative velocities of encounters
        cdef np.ndarray v = draw_maxwellian(v_rms, v_min, v_max, N)
        #Implement encounters
        cdef bool notBound = False
        cdef double a_new, e_new
        for i in range(N):
                (notBound, a_new, e_new) = encounter(m1, m2, v[i], b[i], a, e, M_p)
                if notBound:
                        print('Binary broken!')
                        return (a, e)
                a = a_new
