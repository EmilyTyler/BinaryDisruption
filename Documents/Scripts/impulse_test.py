...
plt.show()
'''
plt.title('Absolute average fractional error in energy change due to impulse approximation (double Bahcall et al. equations)', wrap=True)
ax = plt.gca()
cs = ax.pcolormesh(a_bins/au, b_bins/au, np.transpose(np.absolute(E_frac_avg)), norm=colors.LogNorm(), vmin=0.001, vmax=1000.0)
plt.colorbar(cs)
plt.ylabel('Impact parameter, au')
plt.xlabel('Semi-major axis, au')
plt.xscale('log')
